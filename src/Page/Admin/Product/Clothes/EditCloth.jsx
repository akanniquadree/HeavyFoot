...
export default function EditCloth() {
    ...
    useEffect(()=>{
      ...
      const Clothe = async() =>{
        ...
        if(data){
          setProduct(data.filter((itm)=>String(itm.id) === id)[0])
          setName(product.name)
          setPrice(product.price)
          setDiscount(product.discount)
          setStatus(product.status)
          setQuantity(product.quantity)
          setAvater(product.avater)
    ...
    return () => {ignore = true}
    },[id, product.avater, product.category, product.description, product.discount, product.images, product.name, product.price, product.quantity, product.status])
   ...
   const submitHandler = async(e) =>{
    ...
    try{
      ...
      setDisable(true)
      const data = await axios.post(`https://ecommerces-api.herokuapp.com/api/v1/admin/edit_product/${id}`,formData, {headers: {"Authorization": "Bearer " +localStorage.getItem("admintoken") }})
      if(data){
        setError("")
        ...
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={12} lg={12}>
              ...
              >
                 <form className="fom" onSubmit={submitHandler} encType="multipart/form-data">
                    ...
                    <textarea rows={5} placeholder='Enter the product Information' required value={description} onChange={(e)=>setDescription(e.target.value)}></textarea>
                    <button type='submit' disabled={disable} className="userCreate" onClick={handleClick(SlideTransition)}>{disable ? <CircularProgress thickness={4.5} sx={{color:"white"}} size="13px"/> :"Update Product"}</button>
